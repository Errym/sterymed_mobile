...
This corrects the previous entry in this file and the master plan's ground rule ("the 409 is success, not error") — that description does not match the real backend. `SyncEngine` correctly handles the *benign replay* case regardless: it treats any non-throwing HTTP response (including a replayed 200) as success and removes the item. **Correction (2026-09-22, Phase 5 audit):** the code's explicit 409 branch was still wrong — it auto-removed the item on any 409, which is only safe for a benign replay (which never actually throws a 409 in the first place) and is silent data loss for a genuine key-reuse-with-different-payload. Fixed in `sync_engine.dart` to route 409 through manual review like 422/403. See `docs/DAILY_LOG.md`.
